...
import { FaSignOutAlt, FaTachometerAlt, FaUser, FaTable, FaThLarge, FaNewspaper } from "react-icons/fa";
...
const AdminSideBar = () => {
  ...
  const menuItems = [
    ...
    { to: "KoiGrowthStandard", icon: <FaThLarge />, label: "Koi Growth Standard" },
    { to: "WaterParameterStandard", icon: <FaThLarge />, label: "Water Parameter Standard" },
  ...
  return (
    <div className="flex h-auto">
      <div
        className={`sidebar ${open ? "w-72" : "w-20"} bg-black h-full p-5 pt-8 relative duration-300`}
      >
        <img
          src={controlButton}
        ...
        <ul className="pt-6">
          {menuItems.map((item) => (
            <Link to={item.to} key={item.label}>
              <li
                className={`flex rounded-md p-2 cursor-pointer text-gray-300 text-sm items-center gap-x-4 mt-2 ${location.pathname.includes(item.to)
