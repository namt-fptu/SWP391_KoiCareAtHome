...
const Statistics = () => {
  ...
  const [carbonDioxide, setCarbonDioxide] = useState({ labels: [], datasets: [] });
  ...
  const [waterParametersData, setWaterParametersData] = useState({ labels: [], datasets: [] });
...
const ammoniumDatasets = [
  ...
  return (
    <div className="flex-1 h-full p-5 bg-gray-900 min-h-screen flex flex-col">
      ...
      ) : null} {/* Only show if either pond or fish is selected */}
  ...
  <div className="max-w-sm mx-auto mb-8 ">
    ...
    <div className="flex flex-wrap gap-4 justify-center">
      <button
        ...
        className={`px-4 py-2 rounded-lg ${selectedDataType === "pH" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        pH
      </button>
      <button
        onClick={() => setSelectedDataType('hardness')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "hardness" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Hardness
      </button>
      <button
        onClick={() => setSelectedDataType('temperature')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "temperature" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Temperature
      </button>
      <button
        onClick={() => setSelectedDataType('carbonDioxide')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "carbonDioxide" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Carbon Dioxide
      </button>
      <button
        onClick={() => setSelectedDataType('salt')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "salt" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Salt
      </button>
      <button
        onClick={() => setSelectedDataType('nitrates')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "nitrates" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Nitrates
      </button>
      <button
        onClick={() => setSelectedDataType('nitrites')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "nitrites" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Nitrites
      </button>
      <button
        onClick={() => setSelectedDataType('ammonium')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "ammonium" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Ammonium
      </button>
      <button
        onClick={() => setSelectedDataType('oxygen')}
        className={`px-4 py-2 rounded-lg ${selectedDataType === "oxygen" ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-900"}`}
      >
        Oxygen
      </button>
    </div>
      ...
      <div className="flex flex-col items-center">
        {chartType === "pond" && selectedPond && (
          <div className="w-[700px] bg-white p-5 rounded-lg border-2 border-gray-300">
            {selectedDataType === "temperature" && <LineChart chartData={temperature} />}
            {selectedDataType === "pH" && <LineChart chartData={pHValue} />}
            {selectedDataType === "hardness" && <LineChart chartData={hardness} />}
            {selectedDataType === "carbonDioxide" && <LineChart chartData={carbonDioxide} />}
            {selectedDataType === "salt" && <LineChart chartData={salt} />}
            {selectedDataType === "nitrates" && <LineChart chartData={nitrates} />}
            {selectedDataType === "ammonium" && <LineChart chartData={ammonium} />}
            {selectedDataType === "oxygen" && <LineChart chartData={oxigen} />}
            {selectedDataType === "nitrites" && <LineChart chartData={nitrites} />}
          </div>
        )}
