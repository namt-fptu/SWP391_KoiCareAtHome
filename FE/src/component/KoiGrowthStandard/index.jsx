...
const KoiGrowthStandard = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (editingStandard) {
        ...
        await api.put(
          `koiGrowthStandard/updateKoiStandard/${editingStandard.id}`,
          values
        );
  ...
  const handleVarietyFilterChange = (value) => {
    ...
    if (value) {
      const filtered = growthStandards.filter(
        (standard) => standard.koiVariety === value
      );
  ...
  const handleUpdate = (standard) => {
    ...
    form.setFieldsValue({
      koiVariety: koiVarieties.find((v) => v.label === standard.koiVariety)
        ?.value,
        ...
        <Row gutter={[24, 24]} justify="center" style={{ marginTop: "20px" }}>
          {filteredStandards.map((standard, index) => (
            <Col key={standard.id || index} xs={24} sm={12} md={8} lg={6}>
              ...
              >
                <p>
                  <strong>Stage:</strong> {standard.stage}
                </p>
                <p>
                  <strong>Min Length (cm):</strong> {standard.minLength}
                </p>
                <p>
                  <strong>Max Length (cm):</strong> {standard.maxLength}
                </p>
                <p>
                  <strong>Min Weight (g):</strong> {standard.minWeigth}
                </p>
                <p>
                  <strong>Max Weight (g):</strong> {standard.maxWeigth}
                </p>
                <p>
                  <strong>Min Feed (g):</strong> {standard.minFeed}
                </p>
                <p>
                  <strong>Max Feed (g):</strong> {standard.maxFeed}
                </p>
                <div
                  style={{
                    display: "flex",
